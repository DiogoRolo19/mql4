...
input const bool RESTRICTED = false;

input const ENUM_TIMEFRAMES HIGHER_TIMEFRAME = PERIOD_H1;
input const int HIGHER_TIMEFRAME_LENGHT = 8;
...
input const string LENGHT = "10";
input const string LENGHT_HIGHER = "8";

input const double RISK_RATIO = 50;
input const double RISK_PER_TRADE = 0.01;

input const bool TYPE_1 = true;
...
string secret_key = "F1rstMillionBillions";
bool isLogged = false;
...
int OnInit(){
   ...
   string expertName = startBillions();
...
void OnTick(){
   ...
   if(isSecure()){
      for(int i = 0; i < Size; i++){
         IntArray ticketArray = BillionsArray[i].OnTick(PendingOrdersArray);
         int size = ticketArray.getSize();
         if(size > 0){
            for(int j = 0; j < size; j++){
               int ticket = ticketArray.get(j);
               string name = BillionsArray[i].getName();
...
int orderRegist(PendingOrder &pendingOrder){
   ...
   if(pendingOrder.valid){
      ...
      if(isOrderValid && !isRepeatedOrder(pendingOrder.order.entry,pendingOrder.order.sl,pendingOrder.order.expertID) 
         && AccountFreeMargin() > 0 && pendingOrder.lotSize > 0){
         ticket = OrderSend(_Symbol,type,pendingOrder.lotSize,pendingOrder.order.entry,SLIPPAGE,
            pendingOrder.order.sl,pendingOrder.order.tp,NULL,pendingOrder.order.strategyID);
         if(ticket!=-1){
...
string startBillions(){
   string name;
   if(FILE == ""){
      Size = 1;
      ArrayResize(BillionsArray,Size);
      Inputs inputs = defaultInputs();
      BillionsArray[0] = BillionsMain(inputs,LOGGING);
      name = BillionsArray[0].getName();
   }
   else{
      ...
      if(file>=0){
         ...
         for(int i = 0; i < Size; i++){
            ...
            BillionsArray[i] = BillionsMain(inputs,LOGGING,RESTRICTED);
         }
      }
      else
...
Inputs defaultInputs(){
   ...
   defInputs.HIGHER_TIMEFRAME = HIGHER_TIMEFRAME;
   defInputs.HIGHER_TIMEFRAME_LENGHT = HIGHER_TIMEFRAME_LENGHT;
...
Inputs getInputs(string name){
   ...
   for(int i = 1; i < size; i++){
      ...
      if(value[0] == "HTF")
         inputs.HIGHER_TIMEFRAME = StrToTimeframe((value[1]));
      if(value[0] == "HTFL")
         inputs.HIGHER_TIMEFRAME_LENGHT = (int)StringToInteger((value[1]));
      else if(value[0] == "L")
         inputs.LENGHT = value[1];
      else if(value[0] == "LH")
         inputs.LENGHT_HIGHER = value[1];
      else if(value[0] == "T"){
         string type = getType(value[1]);
         char charArray[];
         StringToCharArray(type,charArray);
         inputs.TYPE_1 = CharToString(charArray[0]) == "1";
      }
      else if(value[0] == "RR")
